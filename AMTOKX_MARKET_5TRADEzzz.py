...
async def webhook():
    ...
    if instrument_id in trade_info:
        current_direction = trade_info[instrument_id].get("direction", None)
        if current_direction:
            if current_direction == direction or direction == "Exit":  
                if direction == "Exit":  
                    close_result = await close_positions()
                    if close_result is not None:
                        return close_result,print(f"Position is {trade_info}")
                    return {'code': 201, 'message': f"Order EXIT DONE for {symbol}"},print(f"Position is {trade_info}")
                else:  
                    return {'code': 200, 'message': '無需執行操作，持倉方向與信號相符'},print(f"Position is {trade_info}")
        
            else:  # 如果方向不同
                # 平倉並刪除trade_info裡的持倉信息
                close_result = await close_positions()
                if close_result is not None:
                    return close_result,print(f"Position is {trade_info}")
        ...
        else:  # 如果trade_info中没有這個交易對的方向信息，直接下單
            if direction != "Exit":  # 避免在没有持倉的情況下執行 "Exit"
                await fn.place_new_order(instrument_id, side, accountAPI, tradeAPI, trade_info, instrument_ids, symbol, direction, trade_info_lock)
                print(f"Position is {trade_info}")

    else:  # 如果trade_info中没有這個交易對，直接下單
        if direction != "Exit":  # 避免在没有持倉的情況下執行 "Exit"
            await fn.place_new_order(instrument_id, side, accountAPI, tradeAPI, trade_info, instrument_ids, symbol, direction, trade_info_lock)
            print(f"Position is {trade_info}")
